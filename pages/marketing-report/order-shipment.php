...
<thead>
  <tr class="headings">
    ...
    <th rowspan="2"><center>Stuffing Date</center> </th>
    <th rowspan="2"><center>Hang Tags</center></th>
    <th colspan="2"><center>WIN</center></th>
...
$no=1;
$mkt_report = $office->mkt_rep_ordership($kd_part,$date_awal,$date_akhir);

foreach ($mkt_report as $mkt_reports) {
?>
  <tr class="even pointer">
    <td width='10%' align="center"><b><?php echo $mkt_reports->wkt_order; ?></b></td>
    <td width='15%'><b><?php echo $mkt_reports->no_order; ?></b></td>
    <td width='10%' align="center">
    <?php
    ### stuffing date
    $stuffing_date = $office->report_ordership_orderwkt(trim($mkt_reports->no_order));
    echo "<b>".$stuffing_date->wkt_kirim."</b>";

    $ship_detail = $office->report_ordership_details($kd_part,trim($mkt_reports->no_order));
    ...
    #  foreach ($ship_detail as $ship_details) {
      ?>
          <!-- <table>
            <td> -->
              <?php
      #             ${"stuffing".$i} = $office->report_stuffing_shipment(trim($ship_details->no_shipment));
      #             echo ${"stuffing".$i}->tgl_stuff;
      #             if (${"stuffing".$i}->tgl_stuff==null) {
      #               echo "-";
      #             }
                  // echo $ship_details->no_shipment;
            ?>
          <!-- </td>
          </table> -->
        <?php
      ...
      if ($ship_detail==true) {
        ...
        ?>
            <table>
              <td>
                <?php
                $ex_winused = explode(",",$ship_details->wins_used);
                $ex_winused2 = explode("-",$ship_details->wins_used);
                if ($ex_winused[1]==true or $ex_winused2[1]==true) {
                  echo "<u><a href='#' class='show-win_used' data-id='".$ship_details->no_shipment."'>WINS</a></u>";
                }else{
                  echo "<div class='green'>".$ship_details->wins_used."</div>";
                }
    ...
    //wkt_shipment
      ...
      if ($ship_detail==true) {
        foreach ($ship_detail as $ship_details) {
          ...
          ?>
            <table>
              <td><?php echo $office->datetime_to_date($ship_details->wkt_shipment); ?></td>
            </table>
          <?php
          }else{
            ?>
    ...
    foreach ($ship_detail as $ship_details) {
      ...
      if ($count3=='1') {
        ?>
          <table>
            <td><?php echo $ship_details->bl; ?></td>
          </table>
        <?php
      }else{
        ?>
